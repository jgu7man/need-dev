...
export class EventoService {
    ...
    postEvento( evento: EventoModel): Observable<any> {
        ...
        return this._http.post(UrlApi.local+'saveEvento', params, {headers: headers});
    }

    postServicio( servicio: ServicioModel): Observable<any> {
        let params = JSON.stringify(servicio);
        let headers = new HttpHeaders().set('Content-Type', 'application/json');
        return this._http.post(UrlApi.local+'saveServicio', params, {headers: headers});
    }

    postDatos( Datos: DatosEvento): Observable<any> {
        let params = JSON.stringify(Datos);
        let headers = new HttpHeaders().set('Content-Type', 'application/json');
        return this._http.post(UrlApi.local+'saveDatos', params, {headers: headers});
    }

    getEventos( userId: string ): Observable<any>{
        let headers = new HttpHeaders().set('Content-Type', 'application/json');
        return this._http.get(UrlApi.local+'getEventos/'+userId, {headers: headers});
    }

    getEventoSolo( id: string ):Observable<any>{
        
        let headers = new HttpHeaders().set('Content-Type', 'application/json');
        return this._http.get(UrlApi.local+'eventoSolo/'+id, {headers: headers});
    }

    getDatosEvento( id: string ):Observable<any>{
        let headers = new HttpHeaders().set('Content-Type', 'application/json');
        return this._http.get(UrlApi.local+'datosEvento/'+id, {headers: headers});
    }

    getServicioEvento( id: string ):Observable<any>{
        let headers = new HttpHeaders().set('Content-Type', 'application/json');
        return this._http.get(UrlApi.local+'servicioEvento/'+id, {headers: headers});
    }
